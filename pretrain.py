...
# pre-train model, compute validation acc after n epoches
if __name__ == '__main__':
    ...
    parser.add_argument('--dataset', type=str, default='TieredImagenet', choices=['MiniImageNet', 'TieredImagenet', 'CIFARFS'])    
    parser.add_argument('--backbone_class', type=str, default='Res18', choices=['ConvNet', 'Res12', 'Res18'])
    parser.add_argument('--schedule', type=int, nargs='+', default=[50, 100], help='Decrease learning rate at these epochs.')#cifar:[50, 100] tiered:[75, 150, 300]
    ...
    if args.dataset == 'CIFARFS':
        args.data_path = '/path/to/dataset/of/CIFAR-FS/'
        args.split_path = './split/cifarfs/' 
    elif args.dataset == 'TieredImagenet':
        args.data_path = '/path/to/dataset/of/tieredminiimagenet/'
        args.split_path = './split/tiered/'     
    save_path = '-'.join([args.dataset, args.backbone_class, 'Pre'])
    ...
    train_loader = DataLoader(dataset=trainset, batch_size=args.batch_size, shuffle=True, num_workers=12, pin_memory=True)
    ...
    val_loader = DataLoader(dataset=valset, batch_sampler=val_sampler, num_workers=12, pin_memory=True)
    args.way = valset.num_class  #16
    args.shot = 1
